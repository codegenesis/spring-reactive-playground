...
@Profile("couchbase")
@Repository
public class CouchbasePersonRepository implements Sink<String, Person> {
    ...
    public Mono<Void> insert(Publisher<Person> personStream) {
        return Flux.from(personStream).flatMap(person -> {
            ...
            EntityDocument<Person> doc = EntityDocument.create(id, person);
    ...
    public Flux<Person> put(Flux<? extends Sink.Record<String, Person>> kafkaRecords) {
        return kafkaRecords
                   .concatMap(record -> {
                           EntityDocument<Person> doc = EntityDocument.create(record.key(), record.value());
                           return RxJava1ObservableConverter.from(this.repository.replace(doc))
                                                            .map(r -> {
